...
def _have_any_master() -> bool:
    """Return True if master parquet parts or legacy CSV exist."""
...
def ensure_secret_seed_download():
    """
    If DATA_URL is set and we have no master/seed, download a seed CSV into seeds/.
...
def _add_enriched_parquet_to_master(parquet_path: str):
    """Copy the parquet produced for this upload into the master dataset folder as a new part."""
...
def _read_master_parquet() -> pd.DataFrame:
    """Load all parquet parts, unify dtypes per column, and concat safely."""
    ...
    for p in part_paths:
        try:
            t = pq.read_table(p)
            for f in t.schema:
                observed.setdefault(f.name, set()).add(_type_tag(f.type))
            good_paths.append(p)
        except Exception as e:
            try: shutil.move(p, os.path.join(qdir, os.path.basename(p)))
    ...
    for name, tags in observed.items():
        if name == "Attack Start Time":
            targets[name] = pa.large_string(); continue
        if tags == {"int"}: targets[name] = pa.int64()
        elif tags == {"float"} or tags == {"int","float"}: targets[name] = pa.float64()
        elif tags == {"ts"}: targets[name] = pa.timestamp("ns")
        elif "str" in tags: targets[name] = pa.large_string()
        elif "ts" in tags: targets[name] = pa.large_string()
        elif tags == {"bool"}: targets[name] = pa.bool_()
        else: targets[name] = pa.large_string()
            ...
            for name, target in targets.items():
                if name in t.column_names:
                    ...
                    if hasattr(col, "chunks"):
                        casted = [pc.cast(ch, target) for ch in col.chunks]
                        arr = pa.chunked_array(casted, type=target)
                    else:
...
def _normalize_and_uniquify_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(cols) != len(set(cols)):
        ...
        for c in cols:
            k = seen.get(c, 0)
            new.append(c if k == 0 else f"{c}.{k}")
            seen[c] = k + 1
        df.columns = new
    else:
        df.columns = cols
...
def calculate_recurrence(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df['Attack Start Time'] = pd.to_datetime(df['Attack Start Time'], errors='coerce')
    g = df.groupby('attack_signature')
    first_rows = g.first().reset_index()
    agg = g['Attack Start Time'].agg(['min','max','count']).reset_index()
    agg.columns = ['attack_signature','First Seen','Last Seen','Recurrence Index']
    agg['Time Frame (hrs)'] = (agg['Last Seen'] - agg['First Seen']).dt.total_seconds() / 3600
    def _avg_gap(row):
        if row['Recurrence Index'] > 1 and row['Time Frame (hrs)'] > 0.01:
            return row['Time Frame (hrs)'] / (row['Recurrence Index'] - 1)
        return None
    agg['Avg Time Between Events (hrs)'] = agg.apply(_avg_gap, axis=1)
    return pd.merge(first_rows, agg, on='attack_signature')
...
def process_log_csv_with_progress(input_path: str, output_path: str, chunksize: int = 100_000, fast_mode: bool = True):
    ...
    try:
        for i, df in enumerate(pd.read_csv(input_path, low_memory=False, chunksize=chunksize)):
            ...
            # ensure text columns are pandas string dtype
            text_cols = ["Addition Info","Threat Name","Threat Type","Threat Subtype",
                         "Source IP","Destination IP","Attacker","Victim",
...
def _bootstrap_seed_data():
    if _have_any_master():  # already have parts or legacy CSV
        return
...
def _merge_extra_columns(grouped: pd.DataFrame, raw_df: pd.DataFrame, threat: str) -> pd.DataFrame:
    ...
    extra = (raw_df[raw_df['Threat Type'].astype(str) == str(threat)][['ds'] + extra_cols]
             .drop_duplicates(subset='ds'))
    merged = pd.merge(grouped, extra, on='ds', how='left')
    le_sev, le_dir = LabelEncoder(), LabelEncoder()
...
def train_xgb_for_threat(master_df: pd.DataFrame, threat: str, test_days: int = 7):
    ...
    feature_cols = ['hour','dayofweek','is_weekend','is_night','weekofyear',
    ...
    for c in ['rolling_mean','rolling_std','rolling_sum24h']:
        if c in sub.columns: feature_cols.append(c)
    ...
    model = XGBRegressor(n_estimators=1000, max_depth=6, learning_rate=0.05,
    ...
    safe_name = re.sub('[^A-Za-z0-9]+', '_', threat)
    model_path = os.path.join(MODELS_DIR, f"xgb_{safe_name}.joblib")
...
def forecast_recursive(master_df: pd.DataFrame, threat: str, horizon_days: int, model_bundle: dict,
    ...
    for _ in range(horizon_hours):
        ...
        row = {"ds": ds_next, "hour": hour, "dayofweek": dayofweek, "is_weekend": is_weekend,
        ...
        if len(ylog_series) >= roll_n:
            ...
            row["rolling_mean"] = np.nan; row["rolling_std"] = np.nan
...
def plot_recent_and_forecast(threat: str, recent_actual: pd.DataFrame, fcst_df: pd.DataFrame,
                             lookback_hours: int = 48, title_suffix: str = ""):
...
if "seed_done" not in st.session_state:
    with st.status("Initializing data (first run only)…", expanded=True) as s:
        try:
            # Only when master is empty, ensure we download & merge seeds
            if not _have_any_master() and not SKIP_BOOTSTRAP:
            ...
            # refresh to show counts immediately
...
with st.sidebar:
    ...
    parts_found = len(glob.glob(os.path.join(MASTER_DS_DIR, "*.parquet")))
    st.caption(f"Master parts found: {parts_found}")
    ...
    if cov:
        ...
        try:
            ...
            with open(snapshot_path, "rb") as f:
                st.download_button("⬇️ Download master.parquet", data=f.read(),
                                   file_name="master.parquet", mime="application/octet-stream")
        except Exception as e:
...
chunksize_opt = st.select_slider("Tamaño de chunk para procesar",
...
if process_btn and uploaded is not None:
    ...
    with open(processed_path, "rb") as f:
        st.download_button("⬇️ Descargar CSV procesado", data=f.read(),
                           file_name=os.path.basename(processed_path), mime="text/csv")
...
# -----------------------
if 'master' in locals() and not master.empty:
    ...
    with c1:
        if st.button("⚙️ Entrenar previamente todos los modelos (opcional)", use_container_width=True):
            def pretrain_models(master_df: pd.DataFrame):
                ...
                if not threats:
                    with st.sidebar: st.info("No threat types found to pretrain."); return
                with st.sidebar:
                ...
                for i, threat in enumerate(threats, start=1):
                    ...
                    with st.sidebar:
                        if bundle is None: st.warning(f"⏭️ Not enough data for {threat}")
                        else: st.write(f"✅ {threat} trained")
                    prog.progress(i/total, text=f"{i}/{total} models ready")
                with st.sidebar: st.success("Pretraining complete.")
...
if 'master' not in locals() or master.empty:
    ...
    lookback_hours = st.select_slider("Historial real para mostrar antes del pronóstico",
                                      options=[24,48,72,96,120,144,168], value=48,
    ...
    if run_btn:
        ...
        for threat in chosen:
            ...
            model_bundle = {"model": saved["model"], "features": saved["features"],
                            "cfg": saved["cfg"], "resid_std_log": saved.get("resid_std_log", 0.10)}
            ...
            if isinstance(fcst, dict) and fcst.get("insufficient_history"):
                st.warning(f"**{threat}**: No hay suficiente historial para {horizon_choice}d "
                           f"(needed ~{fcst['needed']}, available {fcst['available']}). Pruebe un horizonte más corto.")
                continue
            ...
            recent_actual = grouped[(grouped["ds"] >= fcst_start - pd.Timedelta(hours=lookback_hours)) &
                                    (grouped["ds"] < fcst_start)]

            fig = plot_recent_and_forecast(threat, recent_actual, fcst, lookback_hours=lookback_hours, title_suffix=f"(+{horizon_choice}d)")
            st.pyplot(fig)

            safe_name = re.sub('[^A-Za-z0-9]+','_', threat)
            plot_path = os.path.join(PLOTS_DIR, f"{safe_name}_{horizon_choice}d.png")
            fig.savefig(plot_path, dpi=160)
            csv_path = os.path.join(PLOTS_DIR, f"{safe_name}_{horizon_choice}d_fcst.csv")
            ...
            with c1:
                with open(csv_path, "rb") as f:
                    st.download_button("⬇️ Download forecast CSV", data=f.read(),
                                       file_name=os.path.basename(csv_path), mime="text/csv")
            with c2:
                with open(plot_path, "rb") as f:
                    st.download_button("⬇️ Download plot PNG", data=f.read(),
...
- **Funciones extra** (`Severity`, `attack_result_label`, `direction`, `duration`) se fusionan por hora cuando están disponibles.
- **Persistencia**: master incremental en `data/master_parquet/` (partes .parquet).
- **Bootstrapping**: ponga CSV(s) en `seeds/` o defina `CYBER_SEED_CSVS="/path/a.csv,/path/b.csv"`.
""")
